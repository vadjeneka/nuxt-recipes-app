  ...
  <div class="flex flex-col max-w-screen-lg container py-20">
    ...
    <div class="mb-8">
      ...
      <ul class="grid grid-cols-1 md:grid-cols-2 gap-2 text-lg">
        <li v-for="(ingredient, index) in data?.ingredients" :key="`ingredient-${index}`">
          <label class="flex gap-2 items-center">
            <input class="hidden peer" type="checkbox" />
            <div
              class="relative w-6 h-6 rounded-full border-2 border-orange-400 flex items-center justify-center peer-checked:after:absolute peer-checked:after:w-4 peer-checked:after:h-4 peer-checked:after:bg-orange-400 peer-checked:after:rounded-full"
            ></div>
    ...
    <div>
      ...
      <ul class="flex flex-col text-lg gap-4">
        <li v-for="(instruction, index) in data?.instructions" :key="`ingredient-${index}`" class="flex gap-2">
          <span
            class="flex items-center justify-center bg-orange-400 w-7 h-7 rounded-full text-white text-sm"
          >
            {{ index + 1 }}
          </span>
